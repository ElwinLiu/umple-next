...
async function getSvgContent(page: Page): Promise<string> {
  ...
  return page
    .locator('[role="application"] g.graph')
    .first()
    .evaluate((el) => el.closest("svg")?.outerHTML ?? "");
}
...
test.describe("Class diagram display options", () => {
  ...
  test("toggling Attributes OFF hides attributes from diagram", async ({
    page,
  }) => {
  ...
  test("toggling Traits does NOT affect method visibility", async ({
    page,
  }) => {
  ...
  test("toggling Methods does NOT affect attribute visibility", async ({
    page,
  }) => {
    // Toggle methods ON
    await toggleDisplayOption(page, "showMethods");
...
test.describe("Default code display options consistency", () => {
  ...
  test("toggling Traits ON should not change method visibility", async ({
    page,
  }) => {
  ...
  test("methods ON then toggling Traits should show generated methods", async ({
    page,
  }) => {
  ...
  test("toggling Traits ON then Methods ON should show generated methods", async ({
    page,
  }) => {
...
test.describe("ReactFlow mode display options", () => {
  ...
  test("RF default: attributes shown, methods hidden in class nodes", async ({
    page,
  }) => {
  ...
  test("RF toggling Methods ON shows methods in class nodes", async ({
    page,
  }) => {
    const studentNode = page.getByTestId("class-node-Student");
    await expect(studentNode).toBeVisible({ timeout: 10_000 });
  ...
  test("RF toggling Attributes OFF hides attributes from class nodes", async ({
    page,
  }) => {
    const studentNode = page.getByTestId("class-node-Student");
    await expect(studentNode).toBeVisible({ timeout: 10_000 });
  ...
  test("RF and GV show consistent content for default state", async ({
    page,
  }) => {
    // Get RF content
    const studentNode = page.getByTestId("class-node-Student");
  ...
  test("RF and GV show consistent content after toggling Methods ON", async ({
    page,
  }) => {
...
test.describe("Sidebar examples display options parity", () => {
  test("class diagram examples: display toggles work and RF/GV are consistent", async ({
    page,
    request,
  }) => {
    await page.goto("/");
    ...
    const sets = await request.get("/api/examples").then((res) => res.json());
    const firstClassExampleId = sets.find(
      (set: { categoryId: string }) => set.categoryId === "class",
    )?.examples?.[0]?.id;
    expect(firstClassExampleId).toBeTruthy();
    await page
      .getByTestId(`command-item-example-${firstClassExampleId}`)
      .click();
  ...
  test("state diagram examples: display toggles work", async ({
    page,
    request,
  }) => {
    await page.goto("/");
    ...
    await expect(page.getByTestId("command-palette")).toBeVisible();
    const sets = await request.get("/api/examples").then((res) => res.json());
    const firstStateExampleId = sets.find(
      (set: { categoryId: string }) => set.categoryId === "state",
    )?.examples?.[0]?.id;
    expect(firstStateExampleId).toBeTruthy();
    await page
      .getByTestId(`command-item-example-${firstStateExampleId}`)
      .click();
...
test.describe("State diagram display options", () => {
  ...
  test("toggling TransitionLabels ON shows transition labels", async ({
    page,
  }) => {
